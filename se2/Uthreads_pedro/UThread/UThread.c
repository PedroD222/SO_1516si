...
BOOL UtJoin(HANDLE ThreadHandle) {
	...
	if (Uthread == RunningThread) return FALSE;
...
//TODO lista de todas as threads da biblioteca
//para não ter de ir ver todos os joiners
BOOL UtAlive(HANDLE ThreadHandle)
{
	PUTHREAD ut = (PUTHREAD)ThreadHandle;
	PUTHREAD utready;
	if (ut == RunningThread)
		return TRUE;
	PLIST_ENTRY list = AliveThreads.Flink;
	while (list != &AliveThreads)
	{
		utready = CONTAINING_RECORD(&list, UTHREAD, Link);
		if (utready == ut) {
			return TRUE;
		}
		list = list->Flink;
...
VOID UtActivate(HANDLE ThreadHandle) {
	PUTHREAD u = (PUTHREAD)ThreadHandle;
	u->State = READY;
	InsertTailList(&ReadyQueue, &(u)->Link);
}

/*
int UtGetSwitchCount() {
	return switchCount;
}*/
